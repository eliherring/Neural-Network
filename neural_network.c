...
void mat_fill_2x2(Matrix *dst, double aa, double ab, double ba, double bb){
    dst->data[0] = aa;
    dst->data[1] = ab;
    dst->data[2] = ba;
    dst->data[3] = bb;
...
void test_mat(int temp){
    ...
    Matrix m = mat_create(2);
    Matrix *p = &m;
    mat_fill_2x2(p, 1, 0, 0, 1);
    mat_print(p);
    mat_fill_2x2(p, 0, 1, 1, 0);
    mat_print(p);
    mat_fill_2x2(p, 1, 2, 3, 4);
    mat_print(p);
